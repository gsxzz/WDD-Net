...
from models.improvments import *
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='/root/moco/models/yolov9-c-1.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        super().__init__()
        ...
        if isinstance(m, (Detect, DDetect, Segment, DSegment, Panoptic)):
            ...
            try:
                m.stride = torch.tensor(
                    [s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])  # forward on cPu
            except RuntimeError:
                try:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            RepNCSPELAN4, SPPELAN,C2f,RepNCSPELAN4_DCNv3}:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('/root/autodl-tmp/checkpoint_0197.pt', map_location = device)
    # checkpoint = torch.load('/root/Yolov9/runs/train_1/exp/weights/last.pt', map_location = device)

    csd = checkpoint['model']# checkpoint state_dict as FP32
    csd_new=checkpoint['model']
    for value in list(csd.keys()):
        if 'encoder_q.' in value:
            print(value[0])
            new_value = value.replace("encoder_q.", "")
            # 修改参数名
            csd_new[new_value] = csd_new.pop(value)
    #
    torch.save(csd_new, '/root/autodl-tmp/checkpoint_0199.pt')
    ...
    #
    a=model.state_dict()
    print(a["model.1.conv.weight"])
    print(csd_new["model.1.conv.weight"])
    print(a["model.1.conv.weight"]==csd_new["model.1.conv.weight"])
    # for value1, value2 in zip(csd.items(), model.state_dict().items()):
    #     print(value1, value2)
    # Save model
    # ckpt = {
